...
const fromToCodes = flightSchedule.map((entry) => [
  entry.from,
  entry.to,
]);
const airportCodes = airportData.map((entry) => entry.IATA_code);

const allCodesExist = fromToCodes.every(
  ([from, to]) => airportCodes.includes(from) && airportCodes.includes(to)
);
